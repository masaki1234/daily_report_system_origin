            ...
            <tbody>
               ...
               <c:forEach var="following" items="${getMyAllFollowing}" varStatus="status">
                    <tr class="row${status.count % 2}">
                        ...
                        <td class="following_date"><fmt:formatDate value='${following.followd_at}' pattern='yyyy-MM-dd HH:mm:ss' /></td>
                        <td class="report_action">
                            <form method="POST" action="<c:url value='/follow/destroy' />">
                                <input type="hidden" name="id" value="${following.id}" />
                                <button type="submit" name="remove"class="btn-twitter-follow"><i class="fa fa-user-plus"></i> <span>フォロー解除</span></button>
                            </form>
                        </td>
                    </tr>
                </c:forEach>
